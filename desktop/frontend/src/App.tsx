...
import { GetStatus, ListShows } from '../wailsjs/go/main/App';
import { EventsOn } from '../wailsjs/runtime/runtime';
import './App.css';

// Phase 3 layout: sidebar of active shows + status panel. The HTML
// chrome is visible during auth, between rounds, and on drain; mpv's
// render surface covers it during playback. Phase 4 layers chrome
// over mpv via the render API.
...
function App() {
  ...
  useEffect(() => {
    ...
    // a finished show drops out of the active list after advance.
    if (status.phase === 'playing' || status.phase === 'fetching' || status.phase === 'drained') {
      ListShows()
        .then((s: any) => setShows(s ?? []))
        .catch(() => setShows([]));
    }
  }, [status.phase, status.last_advance]);
