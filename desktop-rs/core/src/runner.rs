...
impl Runner {
    ...
    fn run_loop(&self) {
        while !self.stop.is_set() {
            ...
            // just blocks until the queued playlist is exhausted (playlist-pos
            // -> -1), which is robust to in-round navigation (skip/prev).
            if self.player.wait_for_round_end(&self.stop).is_err() {
                return;
            }
            if self.stop.is_set() {
                return;
